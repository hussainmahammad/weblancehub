...
const BackgroundNetwork = () => {
  ...
  useEffect(() => {
    if (svgRef.current) {
      ...
      // Ensure text is invisible and at final position
      if (text) {
        text.style.opacity = '0';
        text.setAttribute('transform', 'translate(0, 0)');
      }

      // Animate W formation with staggered delays
      const animateElements = (elements, delayStep = 50) => {
        elements.forEach((el, index) => {
          setTimeout(() => {
            el.style.transition = 'opacity 1s ease-out, transform 1.5s cubic-bezier(0.25, 0.46, 0.45, 0.94)';
            el.style.opacity = '1';
      ...
      animateElements([...lines, ...dots], 20); // Stagger W formation

      // Fade in text only after W is fully formed (2.2s delay)
      if (text) {
        setTimeout(() => {
          text.style.transition = 'opacity 0.8s ease-out';
          text.style.opacity = '0.9';
        }, 2200); // Strict delay to avoid overlap with W
      }
    }
  }, []);
  ...
  const uniquePoints = Array.from(new Set(allPoints.map(p => `${p.cx},${p.cy}`)))
    .map(str => {
  ...
  return (
    ...
    >
      <defs>
        <radialGradient id="networkGradient" cx="0" cy="0" r="1" gradientUnits="userSpaceOnUse" gradientTransform="translate(960 540) rotate(90) scale(1080 1920)">
      ...
      <g stroke="#E0F2F7" strokeWidth="1.2" strokeOpacity="0.5" transform="translate(1056, 540) scale(1.2)">
        {lines.map((line, i) => (
          <line
            key={`line-${i}`}
            x1={line.x1}
        ...
        {uniquePoints.map((point, i) => (
          <circle
            key={`dot-${i}`} // Fixed JSX key syntax
            cx={point.cx}
            cy={point.cy}
            r={point.r || 2}
            fill="#C8F0FF"
...
// ParticleNetwork with left-to-right flow (unchanged)
  ...
  useEffect(() => {
    ...
    const createParticle = () => {
      particles.push({
        ...
        vx: Math.random() * 0.3 + 0.1, // Move right
    ...
    const animate = () => {
      ...
      for (let i = 0; i < particles.length; i++) {
        ...
        // Respawn particles on left when they exit right
